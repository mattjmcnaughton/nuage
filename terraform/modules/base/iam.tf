...
resource "aws_iam_policy" "access_go_carbon_neutral_ssl_certs" {
  name = "${local.name_prefix}-access-go-carbon-neutral-ssl-certs"
  description = "Allow access to go-carbon-neutral.com ssl certs"
  path = local.iam_default_path
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Resource": [
        "arn:aws:s3:::g-mattjmcnaughton/ssl/go-carbon-neutral.com/*"
      ]
    }
  ]
}
